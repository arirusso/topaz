...
MIDI syncable tempo generation in Ruby
...
A simple application of Topaz is to create a Tempo object to step that sequencer at a given rate.  To use a 130 bpm rate generated internally by your computer, you would use

  @tempo = Topaz::Tempo.new(130) { seq.step }
  
If you wish to use external MIDI clock to control the tempo, you can pass a MIDI input to the Tempo object
...
Whether or not you use an internal or external clock source, the event block will be called at quarter note intervals by default.  If you wish to change this set the option :interval.  In this case, 16th notes will occur at 138 bpm   

  @tempo = Topaz::Tempo.new(138, :interval => 16) { seq.step }
  
You can view the current tempo, which is calculated by Topaz if you're using an external MIDI source.

  @tempo.tempo
    => 132.422000
  
You can run the generator in a background thread by passing :background => true to Tempo#start

  @tempo.start(:background => true)
  
You can pass in a block that will stop the clock when it evaluates to true
